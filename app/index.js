...
app.get("/", (req, res) => {
    ...
    pool.query("select codigo, nombre, url from articulo", (error, data1) => {
        pool.query("select nombre, imagen from proveedor", (error, data2) => {
...
app.post("/registro-t", (req, res) => {
    ...
    pool.query(
        "insert into tendero values (?,?,?,?,?)",
        [correo, nombres, apellidos, hash, telefono],
...
app.post("/registro-p", (req, res) => {
    ...
    pool.query(
        ...
        [correo, nombre, hash, telefono, imagen],
...
app.post("/login-t", (req, res) => {
    ...
    pool.query(
        "select contrasenia, nombres, apellidos from tendero where correo= ?",
        [correo],
        (error, data) => {
            if (error) throw error;
...
app.post("/login-p", (req, res) => {
    ...
    pool.query(
        "select contrasenia, nombre from proveedor where correo= ?",
        [correo],
        (error, data) => {
            if (error) throw error;
...
app.get("/tienda", (req, res) => {
    pool.query(
        "select codigo, nombre, valor, url from articulo",
        (error, data) => {
            if (error) throw error;
...
app.get("/detalle-producto/:codigo", (req, res) => {
    pool.query(
        "select codigo, nombre, valor, descripcion, url from articulo where codigo = ?",
        [req.params.codigo],
        (error, data) => {
            if (error) throw error;
            console.log(data);
...
app.get("/test", (req, res) => {
    pool.query("select * from usuario", function (error, results, fields) {
        ...
        res.send(
            `Datos del usuario: ${nombres}, ${apellidos}, ${correo}, ${contrasenia}, ${edad}, ${telefono}`
        );
    });
});
...
app.get("/test-cookies", (req, res) => {
    ...
    if (session.correo) {
        res.send(
            `Usted tiene una sesion iniciada con el siguiente correo: ${session.correo}`
        );
    } else {
